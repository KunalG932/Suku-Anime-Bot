...
class Var:  
    ...
    FFCODE_4KHDR = getenv("FFCODE_4KHDR") or """ffmpeg -i '{}' -progress '{}' -preset slow -c:v libx264 -s 3840x2160 -pix_fmt yuv420p10le -crf 22 -c:a libopus -b:a 192k -c:s copy -map 0 -ac 2 -ab 192k -vbr 2 -level 5.2 -x264-params "colorprim=bt2020:transfer=smpte2084:colormatrix=bt2020nc" -color_range tv -colorspace bt2020nc -color_primaries bt2020 -color_trc smpte2084 '{}' -y"""
    FFCODE_2K = getenv("FFCODE_2K") or """ffmpeg -i '{}' -progress '{}' -preset medium -c:v libx264 -s 2560x1440 -pix_fmt yuv420p -crf 24 -c:a libopus -b:a 128k -c:s copy -map 0 -ac 2 -ab 128k -vbr 2 -level 4.1 '{}' -y"""
    ...
    QUALS = getenv("QUALS", "360 480 720 1080 2K 4KHDR").split()
